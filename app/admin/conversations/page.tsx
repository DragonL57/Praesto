    ...
    try {
      ...
      if (data.conversations.length > 0) {
        const totalMessages = data.conversations.reduce(
          (sum, conv) => sum + (conv.messagesCount || 0),
          0,
        );
        ...
        const activeToday = data.conversations.filter((conv) => {
          const convDate = new Date(conv.lastActive)
            .toISOString()
            .split('T')[0];
  ...
  const handleVisibilityChange = async (
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          error.error || 'Failed to update conversation visibility',
        );
      }
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Based on usage data</p>
          </CardContent>
        </Card>
      </div>
              ...
              <div className="rounded-md border">
                <table className="w-full">
                  ...
                  <tbody>
                    ...
                    ) : conversations.filter((c) => c.visibility === 'public')
                        .length === 0 ? (
                      <tr>
                        <td colSpan={6} className="py-8 text-center">
              ...
              <div className="rounded-md border">
                <table className="w-full">
                  ...
                  <tbody>
                    ...
                    ) : conversations.filter((c) => c.visibility === 'private')
                        .length === 0 ? (
                      <tr>
                        <td colSpan={6} className="py-8 text-center">
