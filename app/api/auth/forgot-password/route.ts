...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { email } = body;
    console.log(
      '[API /api/auth/forgot-password] Received email from body:',
      email,
    );
    ...
    console.log(
      '[API /api/auth/forgot-password] User lookup result:',
      users.length > 0 ? `User found: ${users[0]?.id}` : 'User not found',
    );

    if (users.length === 0) {
      console.log(
        '[API /api/auth/forgot-password] User not found, returning generic success (security)',
      );
      return NextResponse.json(
        {
          success: true,
    ...
    console.log(
      '[API /api/auth/forgot-password] Token saved to database result:',
      result,
    );

    if (!result) {
      console.error(
        '[API /api/auth/forgot-password] Failed to set password reset token in DB for email:',
        email,
      );
      return NextResponse.json(
    ...
    console.log(
      '[API /api/auth/forgot-password] Attempting to send password reset email to:',
      email,
    );
    const emailSent = await sendPasswordResetEmail(email, resetToken);
    console.log(
      '[API /api/auth/forgot-password] Email sending result:',
      emailSent,
    );

    if (!emailSent) {
      console.error(
        '[API /api/auth/forgot-password] Failed to send password reset email to:',
        email,
      );
      return NextResponse.json(
    ...
    console.log(
      '[API /api/auth/forgot-password] Password reset email sent successfully to:',
      email,
    );
    return NextResponse.json(
    ...
    try {
      ...
      requestBodyForError =
        'Failed to clone or read request body for error logging.';
    }
    console.error(
      '[API /api/auth/forgot-password] Request body during error:',
      requestBodyForError,
    );
    return NextResponse.json(
      {
        success: false,
        message: 'An error occurred in API route while processing request',
      },
      { status: 500 },
    );
  }
