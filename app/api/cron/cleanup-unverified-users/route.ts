...
export async function POST(request: NextRequest) {
  console.log(
    '[API Cron Cleanup] /api/cron/cleanup-unverified-users route hit',
  );
  ...
  if (!CRON_SECRET) {
    console.error(
      '[API Cron Cleanup] CRON_SECRET is not set in environment variables.',
    );
  ...
  if (authToken !== CRON_SECRET) {
    console.warn(
      '[API Cron Cleanup] Unauthorized attempt to access cron job. Provided token:',
      authToken,
    );
    return NextResponse.json(
  ...
  try {
    ...
    console.log(
      `[API Cron Cleanup] Deleting unverified users created before: ${olderThanDate.toISOString()}`,
    );
    ...
    console.log(
      `[API Cron Cleanup] Successfully deleted ${count} old, unverified users.`,
    );
    return NextResponse.json({
      success: true,
      message: `Cleanup successful. Deleted ${count} users.`,
    });
  } catch (error) {
