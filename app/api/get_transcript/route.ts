...
export async function GET(request: NextRequest) {
  try {
    ...
    if (process.env.VERCEL) {
      ...
      const apiUrl = `${baseUrl}/api/get_transcript?videoId=${encodeURIComponent(videoId)}${languages.map(lang => `&languages=${encodeURIComponent(lang)}`).join('')
        }${debug ? '&debug=true' : ''}`;
      ...
      try {
        ...
        try {
          ...
          return NextResponse.json({
            ...
            debugInfo: {
              ...
              parserError: (parseError instanceof Error ? parseError.message : String(parseError))
            },
            transcript: null,
            videoInfo: {},
          }, { status: 500 });
        ...
        return NextResponse.json({
          ...
          detailedError: (fetchError instanceof Error ? fetchError.stack : undefined),
          debugInfo: {
            url: apiUrl,
      ...
      pythonProcess.on('close', (code) => {
        ...
        if (code === 0) {
          try {
            ...
            console.error('Failed to parse Python output as JSON:', (err instanceof Error ? err.message : String(err)));
