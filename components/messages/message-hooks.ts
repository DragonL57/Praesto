...
import type { Message, TextPart, ReasoningPart, MessagePart, ToolCallPart, ToolResultPart } from '@/lib/ai/types';
import type {
...
export function useOrderedMessageParts(message: Message): MergedMessagePart[] {
    return useMemo(() => {
        ...
        message.parts.forEach((part, index) => {
            ...
            // Handle text parts with embedded thinking
                ...
                if (thinkMatch?.[1]) {
                    ...
                    if (thinkingLines.length > 0) {
                        currentReasoningGroup.push(thinkingLines.join('\n').trim());
                        const cleanedText = lines.filter(line => !line.trim().startsWith('>')).join('\n').trim();
                        if (cleanedText.length > 0) {
                            flushReasoningGroup();
                            merged.push({ 
                                type: 'part', 
            ...
            // Handle Tool Call parts
                ...
                if (isReasoningTool(toolCall.toolName)) {
                    ...
                    for (let j = index + 1; j < message.parts.length; j++) {
                        ...
                        if (nextPart.type === 'tool-result' && (nextPart as ToolResultPart).toolCallId === toolCall.toolCallId) {
                            hasResult = true;
                            break;
                        }
                    }
                        ...
                        if (toolCall.toolName === 'executeSandboxCode') {
                            ...
                            currentReasoningGroup.push({
                                ...
                                data: {
                                    ...
                                    state: (toolCall as ToolCallPart & { state?: string }).state === 'input-streaming' ? 'input-streaming' : 'input-available',
                                },
                            });
                ...
                if (isReasoningTool(toolResult.toolName)) {
                    ...
                    if (result && typeof result === 'object') {
                        if (toolResult.toolName === 'think') {
                            ...
                            currentReasoningGroup.push({ type: 'webSearch', data: searchResult as WebSearchData });
                        ...
                        } else if (toolResult.toolName === 'executeSandboxCode' && toolCallInfo) {
        ...
        const partsOnly = merged
            .filter((m): m is { type: 'part'; part: EnhancedMessagePart } => m.type === 'part')
            .map(m => m.part);
    ...
    return useMemo(() => {
        ...
        message.parts.forEach((part, index) => {
            if (part.type === 'reasoning') {
                ...
                const thinkingLines = textPart.text.split('\n')
                    .filter(line => line.trim().startsWith('>'))
                    .map(line => line.trim().substring(1).trim());

                if (thinkingLines.length > 0) elements.push(thinkingLines.join('\n').trim());
            }
            else if (part.type === 'tool-call') {
                const toolCall = part as ToolCallPart & { state?: string };
                if (isReasoningTool(toolCall.toolName)) {
                    toolCallMap.set(toolCall.toolCallId, { index, toolName: toolCall.toolName, args: toolCall.args as Record<string, unknown>, state: toolCall.state });
                }
            }
                ...
                if (result && typeof result === 'object') {
                    if (toolName === 'think') {
                        ...
                        const searchResult = result as { results: WebSearchResult[]; query: string; count: number };
                        ...
                        if (webContentResult.url) {
                            elements.push({ type: 'fetchedPageInfo', data: { url: webContentResult.url, query: webContentResult.query } });
                            filterIndices.add(index);
                            if (toolCallInfo) filterIndices.add(toolCallInfo.index);
                        }
                    }
                    else if (toolName === 'executeSandboxCode' && toolCallInfo) {
                        const executionResult = result as { stdout?: string; stderr?: string; exitCode?: number };
                        const args = toolCallInfo.args as unknown as SandboxToolArgs;
                        elements.push({ type: 'codeExecution', data: { code: args.code, language: args.language, packages: args.packages, stdout: executionResult.stdout, stderr: executionResult.stderr, exitCode: executionResult.exitCode, state: 'output-available' } });
                        filterIndices.add(index);
        ...
        toolCallMap.forEach((info, _id) => {
            if (!filterIndices.has(info.index) && info.toolName === 'executeSandboxCode') {
                const args = info.args as unknown as SandboxToolArgs;
                elements.push({ type: 'codeExecution', data: { code: args.code, language: args.language, packages: args.packages, state: info.state === 'input-streaming' ? 'input-streaming' : 'input-available' } });
 ...
 */
export function useProcessedParts(message: Message, indicesToFilter: Set<number>): EnhancedMessagePart[] {
    return useMemo(() => {
        const thinkTagRegexForReplace = /<think>[\s\S]*?<\/think>/i;
