...
export function AdminSidebar({
  ...
  const [navigation, setNavigation] = useState<NavItem[]>([
    ...
    { name: 'Conversation Analytics', href: '/admin/conversations', icon: MessageSquare, current: false },
    { name: 'Model Usage & Tokens', href: '/admin/tokens', icon: Database, current: false },
    { name: 'System Settings', href: '/admin/settings', icon: Settings, current: false },
    { name: 'Security & Access', href: '/admin/security', icon: Shield, current: false },
    { name: 'Logs & Monitoring', href: '/admin/logs', icon: AlertCircle, current: false },
    { name: 'Billing & Subscription', href: '/admin/billing', icon: CreditCard, current: false },
  ]);
  ...
  useEffect(() => {
    ...
    setNavigation(nav => 
      nav.map(item => ({
        ...item,
        current: item.href === pathname || (item.href === '/admin' && pathname === '/admin')
      }))
    );
  }, [pathname]);
            ...
            <nav className="space-y-1 px-2">
              {navigation.map((item) => (
                ...
                >
                  <item.icon
                    className={cn(
                      item.current ? 'text-primary' : 'text-foreground/70 group-hover:text-foreground',
