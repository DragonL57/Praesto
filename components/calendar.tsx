...
import { Calendar as CalendarIcon, Clock, MapPin, Users, Trash2, CheckCircle2 } from 'lucide-react';
...
export function Calendar({ result }: CalendarProps) {
    ...
    return (
      <div className="w-full bg-destructive/10 border-destructive/20 border rounded-lg p-3 sm:p-4">
        <div className="flex items-start gap-2">
          ...
          <div>
            <h3 className="font-semibold text-sm text-destructive">Calendar Error</h3>
      ...
      <div className="w-full bg-card border border-border rounded-lg overflow-hidden">
        <div className="bg-primary/5 px-3 sm:px-4 py-2 sm:py-2.5 border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-sm text-foreground">Calendar Events</h3>
          </div>
          <p className="text-xs text-muted-foreground mt-0.5 break-words">{listResult.message}</p>
        </div>
        
        {listResult.events.length > 0 ? (
          <div className="divide-y divide-border">
            {listResult.events.map((event, index) => (
              <div key={event.id || index} className="p-2.5 sm:p-3 hover:bg-muted/50 transition-colors">
    ...
    return (
      <div className="w-full bg-card border border-border rounded-lg overflow-hidden">
        ...
        <div className="p-2.5 sm:p-3">
          <div className="space-y-2">
            <h4 className="font-semibold text-sm text-foreground break-words">{eventResult.event!.summary}</h4>
            
            <div className="flex items-start gap-1.5 text-xs text-muted-foreground">
              <Clock className="size-3 shrink-0 mt-0.5" />
              <span className="break-words">
                {format(new Date(eventResult.event!.start), 'MMM d, p')} - {format(new Date(eventResult.event!.end), 'p')}
              </span>
            </div>
            
            {eventResult.event!.htmlLink && (
              <a
                href={eventResult.event!.htmlLink}
                target="_blank"
                rel="noopener noreferrer"
      ...
      <div className="w-full bg-card border border-border rounded-lg overflow-hidden">
        <div className="bg-orange-500/10 px-3 sm:px-4 py-2 sm:py-2.5 border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-sm text-foreground">Event Deleted</h3>
          </div>
          <p className="text-xs text-muted-foreground mt-0.5 break-words">{result.message}</p>
        </div>
        ...
        {deleteResult.deletedEvent && (
          <div className="p-2.5 sm:p-3">
            <p className="text-xs text-muted-foreground">
              <span className="font-medium text-foreground">{deleteResult.deletedEvent.summary}</span> removed from calendar.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="w-full bg-card border border-border rounded-lg overflow-hidden">
        <div className="bg-blue-500/10 px-3 sm:px-4 py-2 sm:py-2.5 border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-sm text-foreground">Available Times</h3>
          </div>
          <p className="text-xs text-muted-foreground mt-0.5 break-words">{result.message}</p>
        </div>
        ...
        {slotsResult.freeSlots.length > 0 ? (
          <div className="p-2.5 sm:p-3">
            <div className="space-y-2">
              {slotsResult.freeSlots.map((slot, index) => (
                <div
                  key={index}
  ...
  return (
    <div className="w-full bg-card border border-border rounded-lg p-2.5 sm:p-3">
      ...
      <p className="text-xs text-muted-foreground mt-1 break-words">{'message' in result ? result.message : 'Calendar operation completed'}</p>
    </div>
  );
}
