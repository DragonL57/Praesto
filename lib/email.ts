...
const templates: Record<EmailTemplate, string> = {
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
      <div style="text-align: center; margin-bottom: 20px;">
        <img src="{{logoUrl}}" alt="UniTaskAI Logo" style="width: 60px; height: 60px;">
        <h1 style="color: #333; font-size: 24px; margin-top: 10px;">Verify Your Email</h1>
      </div>
      <p style="color: #555; line-height: 1.5;">Hello {{name}},</p>
      ...
      <div style="text-align: center; margin: 30px 0;">
        <a href="{{verificationUrl}}" style="background-color: #0070f3; color: white; padding: 12px 24px; text-decoration: none; border-radius: 4px; font-weight: bold;">Verify Email</a>
      </div>
      ...
      <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #e0e0e0; text-align: center; color: #999; font-size: 12px;">
        &copy; {{year}} UniTaskAI. All rights reserved.
      </div>
    </div>
  `,
    'password-reset': `
...
export const sendEmail = async ({ to, subject, template, context }: EmailOptions): Promise<boolean> => {
    try {
        ...
        const compiledTemplate = Handlebars.compile(templates[template]);
