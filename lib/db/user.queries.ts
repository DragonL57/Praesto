...
import { user, type User as ActualDbUser } from './schema';
...
export async function deleteOldUnverifiedUsers(
  ...
  try {
    console.log(
      `[DB Query] Attempting to delete unverified users created before: ${olderThan.toISOString()}`,
    );
    ...
    console.log(
      `[DB Query] Successfully deleted ${result.length} old, unverified users.`,
    );
